...
class RRTNode
{
	public:		
		...
		//sample_world should not be empty. This function takes the sample_world, and modifies the state of each of it's threads randomly.
		void sampleWorld(World* sample_world);
		
		//Nearest_node is set to the node whose world is nearest to sample_world. The nearest node can be this node or any of its children (recursively).
		double nearestNeighbor(RRTNode*& nearest_node, const World* sample_world);
		
		//Extend from current node to a new extended_node whose world is close to sample_thread. The new node is added to this node's children.
