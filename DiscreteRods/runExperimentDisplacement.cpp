...
int main (int argc, char * argv[])
{
  ...
  horizon_trajs.resize(horizon.size());
	...
	vector<double> total_displacements(3, 0.0);
  ...
  for (int h = 0; h < horizon.size(); h++) { 
    ...
    if (horizon[h] == 0) { 
      ...
      for (int i = start_ind+1; i < end_ind; i++) {
        ...
        vector<double> displacements(3, 0.0);
        OLcopy->applyRelativeControl(ctrls[i-1], displacements, noise_thresh, true);
        openLoopWorlds.push_back(new World(*OLcopy));
        
        assert(total_displacements.size() == displacements.size());
        for (int j = 0; j < total_displacements.size(); j++) {
          total_displacements[j] += displacements[j];
        }
      }
      delete OLcopy; 
...
//      sqp_init.push_back(init_worlds);
  ...
  cout << "total_displacements: ";
  for (int i = 0; i < total_displacements.size(); i++) {
    cout << total_displacements[i] << " ";
  }
  cout << endl;

//	sprintf(traj_out_filename, "%s_%.1f_%.2f", traj_out_filename, experiment_timer->elapsed());
