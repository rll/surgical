...
class ThreadConstrained {
	public:
		...
		void getOperableVertices(vector<Vector3d> &operable_vertices, vector<int> &operable_vertices_nums, vector<bool> &constrained_or_free);
		Vector3d start_pos();
		Vector3d end_pos();
		Matrix3d start_rot();
		Matrix3d end_rot();
    ...
    vector<int> constrained_vertices_nums;
		...
		void splitThread(int thread_num, int vertex);
		...
		int threadOwner(int absolute_vertex);
		...
		int localVertex(int absolute_vertex);
};
