...
#ifdef ISOTROPIC
#define TRAJ_BASE_NAME "suturepurple_processed_projected"
#define IMG_BASE_DIR "../../vision/captures/suturepurple"
#else
#define TRAJ_BASE_NAME "ribbon_dots_processed_projected"
#define IMG_BASE_DIR "../../vision/captures/ribbon_dots"
#endif

#define ADD_MINIMIZED_THREAD true /*comment out entire line to remove*/
...
Thread* minimized_thread;
vector<Vector3d> minimized_points;
vector<double> minimized_twist_angles;
vector<Matrix3d> minimized_material_frames;
...
Vector3d minimized_positions[2];
Vector3d minimized_tangents[2];
Matrix3d minimized_rotations[3];
...
*/
  ...
  glColor3d(1.0, 0.0, 0.0); //red
  glVertex3f(0, 0, 0);
  glVertex3f((float)(rotations_project(0,0)), (float)(rotations_project(1,0)), (float)(rotations_project(2,0)));
  glColor3d(0.0, 1.0, 0.0); //green
  glVertex3f(0, 0, 0);
  glVertex3f((float)(rotations_project(0,1)), (float)(rotations_project(1,1)), (float)(rotations_project(2,1)));
  glColor3d(0.0, 0.0, 1.0); //blue
  glVertex3f(0, 0, 0);
  glVertex3f((float)(rotations_project(0,2)), (float)(rotations_project(1,2)), (float)(rotations_project(2,2)));
  glEnd( );
	*/
...
#ifdef ADD_MINIMIZED_THREAD

	//Draw Minimized Thread
  glColor3f (0.5, 0.5, 0.8);

  pts_cpy[minimized_points.size()][3];
  twist_cpy[minimized_points.size()];
  pts_cpy[0][0] = 0.0;
  pts_cpy[0][1] = 0.0;
  pts_cpy[0][2] = 0.0;
  twist_cpy[0] = -(360.0/(2.0*M_PI))*minimized_twist_angles[0];
  //std::cout << twist_cpy[0] << std::endl;

  for (int i=1; i < points.size(); i++)
  {
    pts_cpy[i][0] = minimized_points[i](0)-(double)minimized_positions[0](0);
    pts_cpy[i][1] = minimized_points[i](1)-(double)minimized_positions[0](1);
    pts_cpy[i][2] = minimized_points[i](2)-(double)minimized_positions[0](2);
    twist_cpy[i] = -(360.0/(2.0*M_PI))*minimized_twist_angles[i];
    //std::cout << twist_cpy[i] << std::endl;
  }
 // std::cout << "FINAL TWIST" << twist_cpy[points.size()-2] << std::endl;
  //glePolyCone (points.size(), pts_cpy, 0x0, radii);
...
#ifdef ADD_MINIMIZED_THREAD  
  minimized_thread = new Thread(*thread);
#ifdef ISOTROPIC
  minimized_thread->set_coeffs_normalized(4,12.33,1e-4);
#else
  Matrix2d bend_coeffs = Matrix2d(2,2);
  bend_coeffs << 2.0, 0,
                 0, 0.0001;
  minimized_thread->set_coeffs_normalized(bend_coeffs, 0.00000110, 0.0001);
#endif
  if(thread_ind == 58) {
  minimized_thread->minimize_energy(30000,1e-6,0.2,1e-7);
  }
  minimized_thread->get_thread_data(minimized_points, minimized_twist_angles, minimized_material_frames);

  minimized_positions[0] = minimized_points.front();
  minimized_positions[1] = minimized_points.back();

  minimized_tangents[0] = minimized_points[1] - minimized_points[0];
  minimized_tangents[0].normalize();
  minimized_tangents[1] = minimized_points[minimized_points.size()-1] - minimized_points[minimized_points.size()-2];
  minimized_tangents[1].normalize();
...
void display_ims()
{
  ...
  axis_colors[2] = Scalar(255,0,0);
	...
	for (int frame_ind=0; frame_ind < points.size(); frame_ind++)
	{
		int data_ind;
