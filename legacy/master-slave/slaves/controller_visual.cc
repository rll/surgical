...
int Controller_visual:: control(const double* state, double* goal, double* out){
    ...
    switch(_status){
        ...
        case LQR_CONTROL:
            ...
            return current_lqr_controller()->control(state, goal, out);
        ...
        case VISUAL_SERVOING:
            if(_visual_servo_controller->is_complete(state)){
                ...
                double servo_goal_state[num_actuators];
                _visual_servo_controller->get_goal_state(servo_goal_state);
                hold_position(servo_goal_state,out);
                double ungrip_state[num_actuators];
                create_ungrip_state(_hold_state, ungrip_state);
                create_grip_state(_hold_state, _grip_state);
                _action_controller = new Controller_motion_planner(slave_num, state_size, output_size, _kp, _ki, _kd,  ungrip_state, _grip_state, 2500000); 
        ...
        case ACTING:
            if(_action_controller->is_complete(state)){
                get_return_state();
                _motion_plan_controller = new Controller_motion_planner(slave_num, state_size, output_size, _kp, _ki, _kd, _grip_state, _return_state, 5000000);
...
int Controller_visual:: switch_status(Status status, const double* state, double* out){
    ...
    if(status == LQR_CONTROL)
     ...
     switch(status){
        ...
        case VISUAL_SERVOING:
            printf("Slave %d Beginning Visual Servo.\n", slave_num);
        break;
        
        case ACTING:
            printf("Slave %d initiating grip.\n", slave_num);
        break;
        
        case REVERSE_MOTION_PLANNING:
            printf("Slave %d Pulling Thread.\n", slave_num);
        break;
     ...
     double goal[num_actuators];
     for(int i = 0; i < num_actuators; i++){
        goal[i] = 0;
     }
     return control(state, goal, out);
    //return hold_position(state, out);
}
