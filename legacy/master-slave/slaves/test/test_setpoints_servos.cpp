...
int main(int argc, char** argv) {
	...
	//	char MyPortName[V24_SZ_PORTNAME]={"/dev/ttyS0"};
	Servo_Controller servo_controller = Servo_Controller("/dev/ttyS0");
	int id[] = {11, 14};
	int pos[] = {512, 512};
	int speed[] = {200, 200};
	int num_servos = 2;
	servo_controller.go_to_pos(id, pos, speed, num_servos);
        ...
        if(MASTERS) {
            ...
            while(masters[0]->_accepted && masters[1]->_accepted) {
                ...
                for(int j=0;j<num_slaves;j++){
					...
					if (j == 0)
					{
						if (serial_counter == 15)
						{
							serial_counter = 0;
							double ang_dif[2];
							ang_dif[0] = position.at(iPITCH) - M_PI/2;
							ang_dif[1] = position.at(iTILT) - M_PI/2;
							pos[0] = ((int)(ang_dif[0]*(500/(2*M_PI))))+512;
							pos[1] = ((int)(ang_dif[1]*(500/(2*M_PI))))+512;

							std::cout << "pos1: "<< pos[0] << "   pos2: " << pos[1] << std::endl;
							servo_controller.go_to_pos(id, pos, speed, num_servos);
						}
						serial_counter++;
					}
