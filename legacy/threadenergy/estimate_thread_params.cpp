...
int main (int argc, char * argv[])
{
 ...
 //   params[7] /= GRAV_FACTOR;
  ...
  opt_params_thread_params.orig_params->element(2) = abs(randomMaxAbsValue(CONSTANT_THREAD_DIFF_CURVATURE_ERROR_PENALTY*noise)+CONSTANT_THREAD_DIFF_CURVATURE_ERROR_PENALTY);
  opt_params_thread_params.orig_params->element(3) = abs(randomMaxAbsValue(CONSTANT_THREAD_DIFF_TORSION_ERROR_PENALTY*noise)+CONSTANT_THREAD_DIFF_TORSION_ERROR_PENALTY);
  opt_params_thread_params.orig_params->element(4) = abs((randomMaxAbsValue(GRAVITY_CONSTANT*noise)+GRAVITY_CONSTANT)*1000000.0);
...
//  opt_params_thread_params.orig_params->element(7) = (randomMaxAbsValue(CONSTANT_TOTAL_ERROR_PENALTY*noise)+CONSTANT_TOTAL_ERROR_PENALTY)/1000.0;
  ...
  params[4] = opt_params_thread_params.orig_params->element(2);
  params[5] = opt_params_thread_params.orig_params->element(3);
  params[7] = opt_params_thread_params.orig_params->element(4)/GRAV_FACTOR;
...
*/
  ...
  OPTPP::NLF0 nlp(5, threadParamEvalFunction, threadParamEvalFunction_init, constraints);
  OPTPP::OptPDS objfcn(&nlp);
  objfcn.setFcnTol(1.e-6);
  objfcn.setSSS(25);
...
//  std::cout << (x_sol(8))*1000.0 << std::endl;
  ...
  std::cout << opt_params_thread_params.orig_params->element(2) << std::endl;
  std::cout << opt_params_thread_params.orig_params->element(3) << std::endl;
  std::cout << ((opt_params_thread_params.orig_params->element(4))/1000000.0) << std::endl;
//  std::cout << opt_params_thread_params.orig_params->element(5) << std::endl;
//  std::cout << opt_params_thread_params.orig_params->element(6) << std::endl;
//  std::cout << ((opt_params_thread_params.orig_params->element(7))*1000.0) << std::endl;
...
void threadParamEvalFunction(int ndim, const NEWMAT::ColumnVector& x, double& fx, int& result)
{
  ...
  for(int n = 0; n < opt_params_thread_params.NUM_TRAJ && !skipRest; n++)
  {
    ...
    for (int i=0; i < opt_params_thread_params.savedThreads[n]->size() && !skipRest; i++) //i+= opt_params_thread_params.num_motions_to_sim)
    {
      ...
      //currThread->optimizeManyPoints_startAndEnd_MyParams(opt_params_thread_params.savedThreads[n]->at(i).points, 2, (opt_params_thread_params.savedThreads[n])->at(i).positions, (opt_params_thread_params.savedThreads[n])->at(i).tangents);
...
void prepareOptimizationParams(char * basedir, char * numtraining_file) {
  ...
  opt_params_thread_params.savedThreads.resize(0);
  opt_params_thread_params.threads.resize(0);
  opt_params_thread_params.num_pts_per.resize(0);
...
void evalEnergyParams(int argc, char * argv[])
{
  ...
  if (argc < 4)
  {
    std::cerr << "error: incorrect arguments. please specify:\n" <<
      "eval [params file] [set1 file path] [set2 file path]...\n";
  }

  char params_file[256];
  sprintf(params_file, "%s", argv[2]);

  NEWMAT::ColumnVector paramsToTest(8);

  double* params = new double[8];
  readParams(params_file, params);
  paramsToTest(1) = params[2];
  paramsToTest(2) = params[3];
  paramsToTest(3) = params[4];
  paramsToTest(4) = params[5];
/*  if (params[7] > 10.0)
    paramsToTest(5) = params[7]/GRAV_FACTOR;
  else
    paramsToTest(5) = params[7];
*/
  paramsToTest(5) = params[7]*GRAV_FACTOR;
  ...
  for (int i=1; i <= 8; i++)
  {
    std::cout << paramsToTest(i) << std::endl;
  }
  ...
  opt_params_thread_params.num_threads_total = 0;
  opt_params_thread_params.scoresPerThread.resize(argc-3);
  for(int i = 3; i < argc; i++)
  {
    sprintf(motions_name, "%s_traj.txt", argv[i]);
    sprintf(thread_name,  "%s_traj_imgs.txt", argv[i]);
    sprintf(points_name,  "%s_traj_imgs.points", argv[i]);
    cout << "Reading in trajectories: " << endl;
    Trajectory_Reader* traj_reader = new Trajectory_Reader(motions_name, thread_name, points_name);
    traj_reader->read_motions_from_file();
    traj_reader->read_threads_from_file();
    ...
    //initialize for score tracking per thread
    opt_params_thread_params.scoresPerThread[i-3].resize(opt_params_thread_params.threads.back()->size(),0.0);
  }
  opt_params_thread_params.num_motions_to_sim = NUM_MOTIONS_TO_SIM;
  opt_params_thread_params.orig_params = new NEWMAT::ColumnVector(5);
  opt_params_thread_params.NUM_TRAJ = argc-3;


  double val;
  int result;
  threadParamEvalFunction(5, paramsToTest, val, result);
  cout << paramsToTest(1) << " " << paramsToTest(2);
  std::cout << "HAD A RESULT OF " << val << std::endl;


  //print the per-trajectory data
  char per_sample_scores_file[256];
  sprintf(per_sample_scores_file, "%s_perSamplesScore_eval", argv[3]);
  ofstream per_samples_out(per_sample_scores_file);
  for (int trajNum=0; trajNum < opt_params_thread_params.scoresPerThread.size(); trajNum++)
  {
    for (int threadNum = 0 ; threadNum < opt_params_thread_params.scoresPerThread[trajNum].size(); threadNum++)
