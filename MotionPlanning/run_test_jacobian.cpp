 ...
 srand48(0);
  ...
  for (int num_links = 5; num_links <= 19; num_links += 2) {
    ...
    for (int thread_ind = trajs_start_ind; thread_ind <= std::min(trajs_end_ind, (int)(start_threads.size()-1)); thread_ind++)
    {
      ...
      initial_thread->dynamic_step_until_convergence(dt, M, max_steps);
      ...
      for (int i = 0; i < all_controls.size(); i++) { 
        ...
        copy_thread->dynamic_step_until_convergence(dt, M, max_steps); 
...
void system_identification_jacobian(Thread* thread, MatrixXd& A) { 
  ...
  for (int i = 0; i < 12; i++) { 
    ...
    thread->dynamic_step_until_convergence(dt, M, max_steps);
