...
start = load('saved_starts_35.txt');
goal = load('saved_goals_35.txt');
start_angles = start(:,90);
goal_angles = goal(:,90);

start_pts = zeros(35, 3);
end_pts = zeros(35, 3);
...
closed_loop_sqp = load('CLOSED_LOOP_SQP_35_results.txt');
linearize_only = load('LINEARIZE_ONLY_35_results.txt');
linearize_via_trajectory = load('LINEARIZE_VIA_TRAJECTORY_35_results.txt');
open_loop_sqp = load('OPEN_LOOP_SQP_35_results.txt');
rrt_planner = load('RRT_PLANNER_35_results.txt');
rrt_planner_sqp_closed_loop_smoother = load('RRT_PLANNER_SQP_CLOSEDLOOP_SMOOTHER_35_results.txt');
rrt_planner_sqp_open_loop_smoother = load('RRT_PLANNER_SQP_OPENLOOP_SMOOTHER_35_results.txt');
rrt_planner_sqp_closedloop_to_end = load('RRT_PLANNER_SQP_CLOSEDLOOP_ONLY_LAST_35_results.txt');
rrt_dim1_planner = load('RRT_dim1_PLANNER_35_results.txt');
rrt_dim1_planner_sqp_closed_loop_smoother = load('RRT_dim1_PLANNER_SQP_CLOSEDLOOP_SMOOTHER_35_results.txt');
rrt_dim1_planner_sqp_open_loop_smoother = load('RRT_dim1_PLANNER_SQP_OPENLOOP_SMOOTHER_35_results.txt');
rrt_dim1_planner_sqp_closedloop_to_end = load('RRT_dim1_PLANNER_SQP_CLOSEDLOOP_ONLY_LAST_35_results.txt');
rrt_dim2_planner = load('RRT_dim2_PLANNER_35_results.txt');
rrt_dim2_planner_sqp_closed_loop_smoother = load('RRT_dim2_PLANNER_SQP_CLOSEDLOOP_SMOOTHER_35_results.txt');
rrt_dim2_planner_sqp_open_loop_smoother = load('RRT_dim2_PLANNER_SQP_OPENLOOP_SMOOTHER_35_results.txt');
rrt_dim2_planner_sqp_closedloop_to_end = load('RRT_dim2_PLANNER_SQP_CLOSEDLOOP_ONLY_LAST_35_results.txt');
...
times  = [times, rrt_dim1_planner(:,2), rrt_dim1_planner_sqp_closed_loop_smoother(:,2), rrt_dim2_planner_sqp_open_loop_smoother(:,2), rrt_dim2_planner_sqp_closedloop_to_end(:,2)];
...
table = [mean(scores);std(scores); mean(times)]
h = figure;
hold on;
...
for i = 1:size(plot_1_scores,2)
    bettercdfplot(plot_1_scores(:,i)/34, colorSet(i,:));
end
...
%print(h, '-dpdf', '~/rll-svn/papers/trunk/2011_ISRR_threadplanning/35_scores.pdf', '-r125');
...
plot_2_scores = [plot_2_scores, rrt_dim2_planner(:,1), rrt_dim2_planner_sqp_closedloop_to_end(:,1)];
...
legend('RRT Planner', 'RRT Planner with LCSQP to End', 'DR Planner (n = 1)', 'DR Planner with LCSQP to End (n = 1)','DR Planner (n = 2)', 'DR Planner with LCSQP to End (n = 2)', 'Location', 'Southeast' );  
xlabel('(Distance from Goal)/(Number of Edges)', 'FontSize', 20)
ylabel('Fraction Complete', 'FontSize', 20)
xlim([0,1.5])
set(findobj('type','axes'),'FontSize',16);
%print(h, '-dpdf', '~/rll-svn/papers/trunk/2011_ISRR_threadplanning/35_dr_scores.pdf', '-r125');
