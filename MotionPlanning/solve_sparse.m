...
b(1:size_each_state) = b_data(1:size_each_state) .* -1;
b(end-size_each_state+1: end) = b_data(end-size_each_state+1:end) .* 1;
...
cvx_begin 
    ...
    %expression consecutive_control_diff((num_threads-2)*size_each_control)
    expression u_block(size_each_control*(num_threads-1))
    ...
    %minimize(norm(A*x-b))
    minimize(norm(A*x-b) + 0.001 * norm(u_block,2))
    %minimize(norm(A*x-b) + 0.001 * norm(u_block,1) + 0.0001*norm(consecutive_control_diff,2))
