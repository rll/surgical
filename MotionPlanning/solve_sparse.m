...
weighted_state_diff_constraint = 5e-1;
...
cvx_begin
    ...
    minimize(norm(A*x-b) + norm(edge_violations,2) + norm(consecutive_control_diff, 2) + norm(u,2) + norm(forward_scores)) %+ consectuve_state_diff_weight'*consecutive_state_diff)%norm(consecutive_state_diff,1)) %sum(w)
    subject to
