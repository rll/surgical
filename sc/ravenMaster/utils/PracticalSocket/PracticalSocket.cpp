...
int UDPSocket::setNonblocking() 
	throw(SocketException) 
{
#ifdef WIN32
	u_long mode = 1;
	ioctlsocket(sockDesc, FIONBIO, &mode);
#else
	int opts = fcntl(sockDesc, F_GETFL);
	if (opts < 0) { throw SocketException("UDPSocket::setNonblocking()", true); }
	bool b = true;
	if(b)
		opts |= O_NONBLOCK;
	else
