...
#define IMG_SAVE_BASE "saved_results/curr_set/"
#define IMG_SAVE_BASE_VIS "saved_results/curr_set/visonly_"
#define IMG_SAVE_BASE_BOTHRES "saved_results/curr_set/both_"
#define POINTS_ERR_SAVE_BASE "saved_results/curr_set/points_"
...
enum key_code
{
    NONE, MOVEPOS, MOVETAN, ROTATETAN
};
enum thread_type
{
    black, nylon, purple
...
void processNormalKeys(unsigned char key, int x, int y)
{
    if (key == 't') {
        ...
        if (thread_ind >= 0 && thread_ind < all_threads.size()) {
            glThreads[startThread]->setThread(
                    (new Thread(all_threads[thread_ind])));
            //glutPostRedisplay();
        }
    } else if (key == 'b') {
        thread_ind--;
...
void processKeyUp(unsigned char key, int x, int y)
{
    ...
    key_pressed = NONE;
    move_end[0] = move_end[1] = tangent_end[0] = tangent_end[1]
            = tangent_rotation_end[0] = tangent_rotation_end[1] = 0.0;
}
...
int main(int argc, char * argv[])
{
    ...
    while (thread_ind < all_threads.size()) {
        ...
        points_err_out << thread_ind << " " << err_fullopt << " "
                << err_visiononly << " " << twistAngle_correct << " "
                << score_correct_twist << " " << " " << twistAngle_best << " "
                << score_best_twist << "\n";
...
void DrawStuff(void)
{
    ...
    if (move_end[0] != 0.0 || move_end[1] != 0.0 || tangent_end[0] != 0.0
            || tangent_end[1] != 0.0 || tangent_rotation_end[0] != 0
            || tangent_rotation_end[1] != 0) {
        glThreads[curThread]->ApplyUserInput(move_end, tangent_end,
                tangent_rotation_end);
    }
...
void findThreadInIms()
{
    ...
    glThreads[startThread]->getThread()->get_thread_data(points_real,
            angle_real);
    ...
    err_visiononly = calculate_vector_norm_avg(points_im, points_real)
            / points_im.size();
    ...
    if (thread_vision.optimizeThread()) {
        ...
        thread_vision.clear_display();
        thread_vision.addThreadPointsToDebugImages(Scalar(0, 0, 200),
                glThreads[just_vis_error_thread]->getThread());
        thread_vision.addThreadPointsToDebugImages(Scalar(200, 0, 0));
        thread_vision.add_debug_points_to_ims();
        thread_vision.saveImages(image_save_base_both, thread_ind + 1);

        thread_vision.get_thread_data(points_im, angle_im);
        err_fullopt = calculate_vector_norm_avg(points_im, points_real)
                / points_im.size();
        std::cout << "err: " << err_fullopt << "\t\tvision only: "
                << err_visiononly << std::endl;
        ...
        for (int i = 0; i < thread_vision.twist_scores.size(); i++) {
            if (abs(
                    thread_vision.twist_scores[i].twist_angle
                            - glThreads[startThread]->getThread()->end_angle())
                    < closest_angle_diff) {
                closest_angle_diff
                        = abs(
                                thread_vision.twist_scores[i].twist_angle
...
void addThreadDebugInfo()
{
    ...
    for (int i = 0; i < thread_vision.gl_display_for_debug.size(); i++) {
        ...
        for (int j = 0; j < currP.size; j++) {
            glVertex3f(currP.vertices[j].x - display_start_pos(0),
                    currP.vertices[j].y - display_start_pos(1),
                    currP.vertices[j].z - display_start_pos(2));
        }
        glEnd();
