...
#define IMG_SAVE_BASE "saved_results/"
#define IMG_SAVE_BASE_BOTHRES "saved_results/both_"
#define IMG_SAVE_BASE_VIS "saved_results/visonly_"
#define POINTS_ERR_SAVE_BASE "saved_results/points_"

#define IMG_SAVE_OPENGL "saved_results/opengl_"
...
Trajectory_Reader traj_reader_good("saved_results/savedHypoths_min2/hypoths_nylon_nylon59.txt");
int curr_ind_good = -1;

Trajectory_Reader traj_reader_bad("saved_results/savedHypoths_min/hypoths_nylon_nylon59.txt");
int curr_ind_bad = -1;
...
void processNormalKeys(unsigned char key, int x, int y)
{
    if (key == 't') {
        ...
        key_pressed = ROTATETAN;
            ...
            curr_ind_good++;
            glThreads[1]->setThread((new Thread(traj_reader_good.get_all_threads()[curr_ind_good % traj_reader_good.get_all_threads().size()])));
            } else if (key == 'w') {
                curr_ind_bad++;
                glThreads[2]->setThread((new Thread(traj_reader_bad.get_all_threads()[curr_ind_bad % traj_reader_bad.get_all_threads().size()])));
            }
 ...
 //   findThreadInIms();
                        ...
                        thread_vision._cams->setImageNumber(60);
                        thread_vision.initializeThreadSearch();
                        ...
                        thread_vision.clear_display();
                        glThreads[1]->getThread()->get_thread_data(points, angles);
                        thread_vision.set_vertices(points);
                        thread_vision.addThreadPointsToDebugImages(Scalar(200,0,0));
                        glThreads[2]->getThread()->get_thread_data(points, angles);
                        thread_vision.set_vertices(points);
                        ...
                        thread_vision.saveImages(image_save_base, im_save_ind);
                        glutPostRedisplay();
                        save_opengl_image();
                        } else if (key == 'p') {
    //updateIms();
...
void save_opengl_image()
{
  ...
  //playback and save images
    ...
    char im_name[256];
    sprintf(im_name, "%s-%d.jpg", image_save_base_opengl, im_save_ind+1);
    im_save_ind++;
    imwrite(im_name, img);
    waitKey(1);
  //sleep(0);
