...
#define IMG_SAVE_BASE "saved_results/twist_estimates/"
#define IMG_SAVE_BASE_BOTHRES "saved_results/twist_estimates/both_"
#define IMG_SAVE_BASE_VIS "saved_results/twist_estimates/visonly_"
#define POINTS_ERR_SAVE_BASE "saved_results/twist_estimates/points_"

#define IMG_SAVE_OPENGL "saved_results/twist_estimates/opengl_"



#define FILE_READ_BASE_NYLON "saved_results/savedHypoths_min2/hypoths_nylon_"
#define FILE_READ_BASE_PURPLE "saved_results/savedHypoths_min2/hypoths_purple_"
#define FILE_READ_BASE_BLACK "saved_results/savedHypoths_min2/hypoths_black_"

#define FILE_WRITE_BASE_NYLON "saved_results/twist_estimates/hypoths_nylon_"
#define FILE_WRITE_BASE_PURPLE "saved_results/twist_estimates/hypoths_purple_"
#define FILE_WRITE_BASE_BLACK "saved_results/twist_estimates/hypoths_black_"

#define TWIST_WRITE_BASE "saved_results/twist_estimates/twist_votes_"
#define ANGLE_WRITE_BASE "saved_results/twist_estimates/angle_diffs_"
...
void updateIms(cv::Point3f& start_pt, Vector3d& start_tan, cv::Point3f& end_pt);
...
char file_write_base[256];
char file_read_base[256];
char twist_write_base[256];
char angle_write_base[256];
...
void processNormalKeys(unsigned char key, int x, int y)
{
  ...
  */
  ...
  if (TRY_ALL)
  {
      vector<double> twists_to_try;
      for (double twist=-2.0*M_PI; twist <= (2.0*M_PI + 1e-2); twist += M_PI/12.0)
      {
          twists_to_try.push_back(twist);
          std::cout << "twists to try" << twist << std::endl;
      } 

      vector<double> total_scores(twists_to_try.size());
      for (int i=0; i < total_scores.size(); i++)
      {
          total_scores[i] = 0;
      }

      vector<double> angles_start(twists_to_try.size());
      for (int i=0; i < angles_start.size(); i++)
      {
          angles_start[i] = 0;
      }
      vector<double> angles_end(twists_to_try.size());
      for (int i=0; i < angles_end.size(); i++)
      {
          angles_end[i] = 0;
      }
      ...
      char twist_write_name[256];
      sprintf(twist_write_name, "%s.txt", twist_write_base);

      char angles_write_name[256];
      sprintf(angles_write_name, "%s.txt", angle_write_base);

      ofstream twist_out;
      twist_out.precision(15);
      twist_out.open(twist_write_name);

      ofstream angles_out;
      angles_out.precision(15);
      angles_out.open(angles_write_name);



      for (; thread_ind < 60; thread_ind++)
      {



          char file_read_name[256];
          sprintf(file_read_name, "%s%d.txt", file_read_base, thread_ind);


          Trajectory_Reader traj_reader_threads(file_read_name);
          traj_reader_threads.read_threads_from_file();
          for (int i=0; i < std::min((int)traj_reader_threads.get_all_threads().size(), 10); i++)
          {
              if (i != 0)
                  break;

/*
glThreads[startThread]->setThread((new Thread(traj_reader_threads.get_all_threads()[i])));
glThreads[optimize_thread]->setThread((new Thread(traj_reader_threads.get_all_threads()[i])));
glThreads[just_vis_error_thread]->setThread((new Thread(traj_reader_threads.get_all_threads()[i])));

glThreads[optimize_thread]->getThread()->set_twist_and_minimize(-M_PI/2.0);
glThreads[just_vis_error_thread]->getThread()->set_twist_and_minimize(M_PI/2.0);
glutPostRedisplay();
*/

vector<double> twist_scores;
#ifdef NYLON

if (thread_ind < 10) {
    traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(2048, 5792.6, 1e-4);
} else if (thread_ind < 20) {
    traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(4096, 9742, 1e-4);
} else if (thread_ind < 30) {
    traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(16, 32, 1e-4);
} else if (thread_ind < 40) {
    traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(1024, 3444, 1e-4);
} else if (thread_ind < 50) {
    traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(128, 128, 1e-4);
} else if (thread_ind < 60) {
    traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(8, 4, 1e-4);
}

         // traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(2048, 5792, 1e-4);

        //traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(232, 847, 1e-4);
#elif defined PURPLE
if (thread_ind < 10) {
    traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(4, 6.7272, 1e-4);
} else if (thread_ind < 20) {
    traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(4, 3.3636, 1e-4);
} else if (thread_ind < 30) {
    traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(4.0, 1.4142, 1e-4);
} else if (thread_ind < 40) {
    traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(4.0, 0.25, 1e-4);
} else if (thread_ind < 50) {
    traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(64.0, 512, 1e-4);
} else if (thread_ind < 60) {
    traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(64.0, 1024, 1e-4);
}

        //traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(4, 8, 1e-4);

#elif defined BLACK

if (thread_ind < 10) {
    traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(16384, 11585, 1e-4);
} else if (thread_ind < 20) {
    traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(512, 181, 1e-4);
} else if (thread_ind < 30) {
    traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(4096, 1722, 1e-4);
} else if (thread_ind < 40) {
    traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(2048, 724, 1e-4);
} else if (thread_ind < 50) {
    traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(1024, 512, 1e-4);
} else if (thread_ind < 60) {
    traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(16380, 262140, 1e-4);
}

        //traj_reader_threads.get_all_threads()[i].set_coeffs_normalized(16384, 9742, 1e-4);
#endif
traj_reader_threads.get_all_threads()[i].evaluate_twist_scores(twists_to_try, twist_scores, angles_start, angles_end);

std::cout << "twist scores: " << thread_ind << std::endl;
for (int j=0; j < twist_scores.size(); j++)
{
    twist_out << twist_scores[j] << "   ";
    total_scores[j] += twist_scores[j];
    std::cout << twists_to_try[j] << "\t" << twist_scores[j] << "\t" << angles_start[j] << "\t" << angles_end[j] << std::endl;

    angles_out << angles_start[j] << "   " << angles_end[j] << "   ";
}
twist_out << std::endl;
angles_out << std::endl;
twist_out.flush();
...
std::cout << "total scores: " << thread_ind << std::endl;
for (int j=0; j < total_scores.size(); j++)
{
    std::cout << twists_to_try[j] << "\t\t" << total_scores[j] << std::endl;
}
...
void InitThread(int argc, char* argv[])
{
    ...
    if (_thread_type == nylon)
    {
        ...
        sprintf(file_write_base, "%s%s", FILE_WRITE_BASE_NYLON, "nylon");
        sprintf(file_read_base, "%s%s", FILE_READ_BASE_NYLON, "nylon");
        sprintf(twist_write_base, "%s%s", TWIST_WRITE_BASE, "nylon");
        sprintf(angle_write_base, "%s%s", ANGLE_WRITE_BASE, "nylon");
        ...
        sprintf(file_write_base, "%s%s", FILE_WRITE_BASE_PURPLE, "purple");
        sprintf(file_read_base, "%s%s", FILE_READ_BASE_PURPLE, "purple");
        sprintf(twist_write_base, "%s%s", TWIST_WRITE_BASE, "purple");
        sprintf(angle_write_base, "%s%s", ANGLE_WRITE_BASE, "purple");
        ...
        sprintf(file_write_base, "%s%s", FILE_WRITE_BASE_BLACK, "black");
        sprintf(file_read_base, "%s%s", FILE_READ_BASE_BLACK, "black");
        sprintf(twist_write_base, "%s%s", TWIST_WRITE_BASE, "black");
        sprintf(angle_write_base, "%s%s", ANGLE_WRITE_BASE, "black");
    }
...
void updateIms(Point3f& start_pt, Vector3d& start_tan, Point3f& end_pt)
{
    glThreads[startThread]->updateThreadPoints();
...
void findThreadInIms()
{
    ...
    updateIms(_start_pt, _start_tan, _end_pt);
    thread_vision.setInitPt(_start_pt);
    thread_vision.setEndPt(_end_pt);
    thread_vision.setInitTan(_start_tan);
...
score_best_twist = best_score;
*/
char file_write_name[256];
sprintf(file_write_name, "%s%d.txt", file_write_base, thread_ind);
thread_vision.write_hypoths_to_file(file_write_name);


}

glThreads[optimize_thread]->printThreadData();
}
