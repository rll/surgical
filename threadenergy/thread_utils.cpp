...
double avgDistBetweenPoints(MatrixXd& a, MatrixXd& b)
{
  ...
  {
    toRtn += (a.block(i,0,1,numCols) - b.block(i,0,1,numCols)).squaredNorm();
  }
  toRtn /= ((double)(a.rows()));
...
void optimize_FDNLF(int numParams, OPTPP::USERFCN0 eval, OPTPP::INITFCN init, NEWMAT::ColumnVector& solution, double fcnTol, int numIters, double maxStep)
{
...
void readMatrix4d(Matrix4d& toModify, ifstream& in) {
  for(int i = 0; i < 4; i++) {
    for(int j = 0; j < 4; j++) {
      in >> toModify(i,j);
    }
...
void writeMatrix4d(const Matrix4d& toWrite, ofstream& out) {
  for(int i = 0; i < 4; i++) {
    for(int j = 0; j < 4; j++) {
      out << toWrite(i,j) << " ";
    }
...
void readVector3d(Vector3d& toModify, ifstream& in) {
  for(int i = 0; i < 3; i++) {
    in >> toModify(i);
  }
}

void writeVector3d(const Vector3d& toWrite, ofstream& out) {
  for(int i = 0; i < 3; i++) {
    out << toWrite(i) << " ";
  }
}
...
void readParams(std::string file, double* out) {
  ...
  if (!in)
  {
    cout << "file does not exist: " << file << std::endl;
    exit(0);
  }
...
void writeParams(std::string file, double* towrite) {
  ...
  if (!out)
  {
    cout << "file could not be opened: " << file << std::endl;
    exit(0);
  }
  cout << file.c_str() << endl;
  out << "ROTATION_PENALTY " << towrite[0] << endl
      << "POSITION_PENALTY " << towrite[1] << endl
      << "CURVATURE_PENALTY " << towrite[2] << endl
      << "TORSION_PENALTY " << towrite[3] << endl
      << "DIFF_CURVATURE_PENALTY " << towrite[4] << endl
      << "DIFF_TORSION_PENALTY " << towrite[5] << endl
      << "TOTAL_ERROR_PENALTY " << towrite[6] << endl
      << "GRAVITY " << towrite[7] << endl;
  out.close();
}
