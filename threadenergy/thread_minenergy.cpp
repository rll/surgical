...
Thread::Thread(double length, Vector3d* positions, Vector3d* tangents)
{
  ...
  _numPieces = 2;
	...
	//we want to normalize the length of the Thread, so each piece is in "canonical" form
	threadList = new ThreadPiece(2.0, 0.5, _length/2.0);
  ThreadPiece* threadNext = new ThreadPiece(-1.0, 0.0, _length/2.0);
  threadList->addSegmentAfter(threadNext);
...
Thread::Thread(ThreadPiece* allPieces, Matrix4d& startTrans, double length)
{
  _length = length;
  threadList = allPieces;
  _positions[0] = startTrans.corner(Eigen::TopRight,3,1);
  _tangents[0] = startTrans.corner(Eigen::TopLeft,3,1);
  double Z,Y;
  eulerAnglesFromTransform(startTrans.corner(Eigen::TopLeft,3,3), Z,Y,_angle_first_rot);

  _numPieces = 0;
  Matrix4d endTransform = startTrans;
  endTransform.corner(Eigen::TopLeft,3,3) *= _length;
  ThreadPiece* currPiece = allPieces;
  while (currPiece != NULL)
  {
    endTransform *= currPiece->_transform;
    _numPieces++;
    currPiece = currPiece->_next_segment;
  }

  _positions[1] = endTransform.corner(Eigen::TopRight,3,1);
  _tangents[1] = endTransform.corner(Eigen::TopLeft,3,1);
  setConstraints(_positions, _tangents);
...
//  cout << "what's up with that" << endl;
  double* params = new double[8];
  readParams("../config/test.params", params);
  rotation_error_penalty = params[0];
  position_error_penalty = params[1];
  thread_curvature_error_penalty = params[2];
  thread_torsion_error_penalty = params[3];
  thread_diff_curvature_error_penalty = params[4];
  thread_diff_torsion_error_penalty = params[5];
  total_error_penalty = params[6];
  gravity_penalty = params[7];
  delete [] params;
//  cout << "Rereading params, gravity: " << params[7] << endl;
}

void Thread::initEnergyParamsFromFile(char* fileName)
{
  double* params = new double[8];
...
void Thread::getWantedStartPosition(Vector3d& endPos)
{
  endPos = _positions[0];
}

void Thread::getWantedStartTangent(Vector3d& endTan)
{
  endTan = _tangents[0];
}
...
void Thread::getActualStartPosition(Vector3d& endPos)
{
  endPos = _positions[0];
}

void Thread::getActualStartTangent(Vector3d& endTan)
{
  endTan = _tangents[0];
}
...
void Thread::minimize_energy_fixedPieces()
{

  //optimize new thread
  setStationaryOptParams();
  if (_numPieces != NUM_PIECES_FIXED)
  {
    upsample_numPieces(NUM_PIECES_FIXED);
  }

	opt_params.length_per_segment = 1.0/((double)_numPieces);;
	opt_params.num_segments = _numPieces;
  ...
  //setup OPT++ problem
  NEWMAT::ColumnVector x_sol;
  optimize_FDNLF(2*opt_params.num_segments+1, energyEvalFunction_fixedPieces, energyEvalFunction_init, x_sol, 1.e-6, 1000, 1.0);
  ...
  for (int i=0; i < opt_params.num_segments; i++)
  {
    currPiece->setParams(x_sol.element(2*i), x_sol.element(2*i+1), opt_params.length_per_segment);
    currPiece = currPiece->_next_segment;
  }
  _angle_first_rot = x_sol(opt_params.num_segments*2+1);
...
void Thread::upsample_numPieces(int numPieces)
{
  vector<double> curvatures;
  vector<double> torsions;
  curvatures.resize(numPieces);
  torsions.resize(numPieces);
  for (int i=0; i <curvatures.size(); i++)
  {
	...
	threadList->getParams(0.0, 1.0/((double)numPieces), 0, curvatures, torsions);

  delete threadList;
  double lengthPerPiece = 1.0/((double)numPieces);
  threadList = new ThreadPiece(curvatures[0], torsions[0], lengthPerPiece);
  ThreadPiece* currPiece = threadList;
  for (int i=1; i < numPieces; i++)
  {
    currPiece->addSegmentAfter(new ThreadPiece(curvatures[i], torsions[i], lengthPerPiece));
    currPiece = currPiece->_next_segment;
  }
...
void Thread::upsampleAndOptimize_minLength(double minLength)
{
  ...
  int _numPieces = (int)(1.0/minLength);
  ...
  //setup OPT++ problem
  NEWMAT::ColumnVector x_sol;
  optimize_FDNLF(2*opt_params.num_segments+1, energyEvalFunction, energyEvalFunction_init, x_sol, 1.e-6, 1000);
...
void Thread::setStationaryOptParams(bool addNoise)
{
  if (_noiseAmount != 0.0 && addNoise)
  {
	...
	opt_params.position_end = (1.0/_length)*(_positions[1]+_currNoisePoints[1] - _positions[0]+_currNoisePoints[0]);
  if (opt_params.position_end.norm() >0.999999999)
  {
    opt_params.position_end *= 0.999999999/opt_params.position_end.norm();
  }
...
void Thread::setStationaryOptParams_manyPoints()
{

	opt_params_many_points.position_end = (1.0/_length)*(_positions[1] - _positions[0]);
  if (opt_params_many_points.position_end.norm() > 0.999999999) //to ensure we don't crash if length not enough for points
  {
    opt_params_many_points.position_end *= 0.999999999/opt_params_many_points.position_end.norm();
  }

  opt_params_many_points.tangent_start = _tangents[0];
	opt_params_many_points.tangent_end = _tangents[1];

	opt_params_many_points.tangent_start.normalize();
	opt_params_many_points.tangent_end.normalize();

  opt_params_many_points.gravity_multipler = (gravity_penalty)*_length;
  opt_params_many_points.total_length = _length;

  opt_params_many_points.thread_curvature_error_penalty = thread_curvature_error_penalty;
  opt_params_many_points.thread_torsion_error_penalty = thread_torsion_error_penalty;
  opt_params_many_points.thread_diff_curvature_error_penalty = thread_diff_curvature_error_penalty;
  opt_params_many_points.thread_diff_torsion_error_penalty = thread_diff_torsion_error_penalty;

  opt_params_many_points.thread_position_error_penalty = position_error_penalty;
  opt_params_many_points.thread_rotation_error_penalty = rotation_error_penalty;
  opt_params_many_points.thread_total_error_penalty = total_error_penalty;

  opt_params_many_points.thread_intermediate_position_error_penalty = MANYPOINTS_COEFF_POINT;
//  opt_params_many_points.thread_intermediate_position_exp_error_penalty = MANYPOINTS_COEFF_POINT_EXPONENTIAL;
...
void Thread::toStream(ofstream& out) {
  out << _angle_first_rot << " " << _length << " ";

  writeVector3d(_positions[0], out);
  writeVector3d(_positions[1], out);
  writeVector3d(_tangents[0], out);
  writeVector3d(_tangents[1], out);

  writeMatrix4d(_transform_to_start, out);
  writeMatrix4d(_translate_to_start, out);
  writeMatrix4d(_transform_to_unit, out);
  out << _numPieces << " ";


  ThreadPiece* toCopyPiece = threadList;
  while(toCopyPiece != NULL) {
    out << toCopyPiece->_curvature << " "
	<< toCopyPiece->_torsion << " "
	<< toCopyPiece->_length << " ";
    toCopyPiece = toCopyPiece->_next_segment;
  }

  out << rotation_error_penalty << " "
      << position_error_penalty << " "
      << thread_curvature_error_penalty << " "
      << thread_torsion_error_penalty << " "
      << thread_diff_curvature_error_penalty << " "
      << thread_diff_torsion_error_penalty << " "
      << total_error_penalty << " "
      << gravity_penalty << endl;
}

void Thread::fromStream(ifstream& in) {
  in >> _angle_first_rot >> _length;

  readVector3d(_positions[0], in);
  readVector3d(_positions[1], in);
  readVector3d(_tangents[0], in);
  readVector3d(_tangents[1], in);
  readMatrix4d(_transform_to_start, in);
  readMatrix4d(_translate_to_start, in);
  readMatrix4d(_transform_to_unit, in);
  in >> _numPieces;

  double _curvature, _torsion, _len;
  in >> _curvature >> _torsion >> _len;
  threadList = new ThreadPiece(_curvature, _torsion, _len);
  ThreadPiece* curThread = threadList;
  for(int i = 0; i < _numPieces-1; i++) {
    in >> _curvature >> _torsion >> _len;
    curThread->_next_segment = new ThreadPiece(_curvature, _torsion, _len);
    curThread = curThread->_next_segment;
  }
  in >> rotation_error_penalty
      >> position_error_penalty
      >> thread_curvature_error_penalty
      >> thread_torsion_error_penalty
      >> thread_diff_curvature_error_penalty
...
void Thread::optimizeManyPoints(MatrixXd& constraints, int num_pts_between, vector<double>&curvatures_resampled, vector<double>& torsions_resampled, Matrix4d& start_transform)
{
  ...
  eulerAnglesFromTransform(start_transform.corner(Eigen::TopLeft,3,3), eulerZ, eulerY, eulerX);
...
void Thread::optimizeManyPoints_startAndEnd_MyParams(MatrixXd& constraints, int num_pts_between, Vector3d* positions, Vector3d* tangents)
{
  MatrixXd myPoints(constraints.rows(), 3); //really just going to toss this out...but this gives us curve params
  vector<double> curvatures_resampled;
  vector<double> torsions_resampled;
	//threadList->getParams(0.0, 1.0/((double)numPieces), 0, curvatures_resampled, torsions_resampled);
  resamplePointsAndParams(myPoints, curvatures_resampled, torsions_resampled);

  setConstraints(positions, tangents);
  setStationaryOptParams_manyPoints();


  Matrix4d start_transform;
  getStartTransform(start_transform);

  optimizeManyPoints_startAndEnd(constraints, num_pts_between, curvatures_resampled, torsions_resampled, start_transform);
}

void Thread::optimizeManyPoints_startAndEnd(MatrixXd& constraints, int num_pts_between, vector<double>&curvatures_resampled, vector<double>& torsions_resampled, Matrix4d& start_transform)
{
  int num_constraints = constraints.rows()-1;
  int num_pieces_reopt = num_pts_between*num_constraints;
  double length_per_piece = 1.0/( num_pieces_reopt);
  ...
  opt_params_many_points.orig_params_each_piece->element(2*num_pieces_reopt) =
    acos(_tangents[0](2)/sqrt(_tangents[0](0)*_tangents[0](0) + _tangents[0](1)*_tangents[0](1) + _tangents[0](2)*_tangents[0](2))); 
  opt_params_many_points.orig_params_each_piece->element(2*num_pieces_reopt+1) = atan2(_tangents[0](1), _tangents[0](0));
  opt_params_many_points.orig_params_each_piece->element(2*num_pieces_reopt+2) = _angle_first_rot;



  //opt_params_many_points.orig_params_each_piece->element(2*num_pieces_reopt) = _angle_first_rot;
  ...
  optimize_FDNLF(2*opt_params_many_points.num_segments+3, energyEvalFunctionManyPoints_startAndEnd_freeTans, energyEvalFunctionManyPoints_init, x_sol);
  ...
  _tangents[0](0) = sin(x_sol(2*num_pieces_reopt+1))*cos(x_sol(2*num_pieces_reopt+2));
  _tangents[0](1) = sin(x_sol(2*num_pieces_reopt+1))*sin(x_sol(2*num_pieces_reopt+2));
  _tangents[0](2) = cos(x_sol(2*num_pieces_reopt+1));
  _angle_first_rot = x_sol(2*num_pieces_reopt+3);
  ...
  if (axis.norm() != 0.0)
		axis.normalize();
  Matrix3d rot_to_start(Eigen::AngleAxisd(acos(Vector3d::UnitX().dot(_tangents[0])),axis));
  Matrix3d first_rotation_transform(Eigen::AngleAxisd(_angle_first_rot, _tangents[0]));

  Matrix4d transform_back = Matrix4d::Identity();
  transform_back.corner(Eigen::TopLeft,3,3) = first_rotation_transform*rot_to_start;
  transform_back.corner(Eigen::TopRight,3,1) = _positions[0];

  _tangents[0] = transform_back.corner(Eigen::TopLeft,3,1);
  _tangents[0].normalize();
  //std::cout << "new init tan: \n" << _tangents[0] << std::endl;
  //_tangents[0].normalize();
  ...
  //_tangents[1] = transform_back.corner(Eigen::TopLeft,3,1);
  //_tangents[1].normalize();
  //std::cout << "new end tan: \n" << _tangents[1] << std::endl;
  setConstraints(_positions, _tangents);


  transform_back.corner(Eigen::TopLeft,3,3) /= _length;
  //std::cout << "calc end transform: \n" << transform_back << std::endl;
  ...
  double point_error_energy = 0.0;
	...
	for (int i=1; i <= (ndim-3)/2; i++)
	{
      ...
      if (i%opt_params_many_points.num_pts_between == 0)
      {
        ...
        point_error_energy += pow(  ((new_end_position - opt_params_many_points.points.at(i/opt_params_many_points.num_pts_between-1)).norm()),2);
      }
	}
...
void energyEvalFunctionManyPoints_startAndEnd(int ndim, const NEWMAT::ColumnVector& x, double& fx, int& result)
{
	...
	//error from curvature
	fx = 0.0;
	for (int i=1; i <= ndim-1; i+=2)
	{
		fx += x(i)*x(i);
	}
	fx *= opt_params_many_points.thread_curvature_error_penalty*opt_params_many_points.length_per_segment*opt_params_many_points.total_length;

  //error from torsion
  double torsion_energy = 0.0;
	for (int i=2; i <= ndim-1; i+=2)
	{
		torsion_energy += x(i)*x(i);
	}
	fx += torsion_energy*opt_params_many_points.thread_torsion_error_penalty*opt_params_many_points.length_per_segment*opt_params_many_points.total_length;
  //std::cout << "thread energy: " << fx << std::endl;

  //energy from diff curvature
  double diff_curvature_energy = 0.0;
  for (int i=3; i <= ndim-1; i+=2)
  {
    diff_curvature_energy += pow(x(i)-x(i-2),2);
  }
  ...
  //energy from diff torsion
  double diff_torsion_energy = 0.0;
  for (int i=4; i <= ndim-1; i+=2)
  {
    diff_torsion_energy += pow(x(i)-x(i-2),2);
  }

  fx += (opt_params_many_points.thread_diff_curvature_error_penalty*diff_curvature_energy
         +opt_params_many_points.thread_diff_torsion_error_penalty*diff_torsion_energy)*opt_params_many_points.length_per_segment*opt_params_many_points.total_length;

  double grav_energy = 0.0;
  double point_error_energy = 0.0;
	//calculate new end transform
	Eigen::Transform3d first_rotation_transform(Eigen::AngleAxisd(x(ndim), opt_params_many_points.tangent_start));
	Matrix4d transform_to_end = first_rotation_transform*opt_params_many_points.transform_back;
  ...
  double curr_z = transform_to_end(2,3);
	...
	for (int i=1; i <= (ndim-1)/2; i++)
	{
			...
			//twist_total += opt_params.length_per_segment*x(2*i);
			transform_to_end *= nextTransform;
      grav_energy += ((transform_to_end(2,3)-curr_z)/2.0 + curr_z)*opt_params_many_points.length_per_segment*opt_params_many_points.total_length;
      curr_z = transform_to_end(2,3);
 ...
 // std::cout << "grav energy " << grav_energy*opt_params.gravity_multipler << std::endl;
  fx += grav_energy*opt_params_many_points.gravity_multipler +
          point_error_energy*opt_params_many_points.thread_intermediate_position_error_penalty;
	...
	//error in position
	double pos_err = opt_params_many_points.thread_position_error_penalty*(pow( ((new_end_position - opt_params_many_points.position_end).norm()),2));

	//std::cout << "position error: " << new_end_position << " - " << opt_params.position_end << " ->" << pos_err << std::endl;

	//error in rotation
	double rot_err = exp(opt_params_many_points.thread_rotation_error_penalty*((1.0-opt_params_many_points.tangent_end.dot(new_end_tangent))) );
	//double rot_err = exp(CONSTANT_ROTATION_ERROR_PENALTY*((1.0-opt_params.tangent_start.dot(opt_params.tangent_start)) + (1.0-opt_params.tangent_end.dot(new_end_tangent))) );

	fx += opt_params_many_points.total_length*opt_params_many_points.thread_total_error_penalty*(exp(pos_err + rot_err) );
...
void energyEvalFunctionManyPoints_startAndEnd_freeTans(int ndim, const NEWMAT::ColumnVector& x, double& fx, int& result)
{
  ...
  //create new start transform
  Vector3d newTan(sin(x(ndim-2))*cos(x(ndim-1)), sin(x(ndim-2))*sin(x(ndim-1)), cos(x(ndim-2)));
	Vector3d axis = Vector3d::UnitX().cross(newTan);
	if (axis.norm() != 0.0)
		axis.normalize();
  Matrix3d rot_to_start(Eigen::AngleAxisd(acos(Vector3d::UnitX().dot(newTan)),axis));
	Matrix3d first_rotation_transform(Eigen::AngleAxisd(x(ndim), newTan));

  Matrix4d transform_to_end = Matrix4d::Identity();
  transform_to_end.corner(Eigen::TopLeft,3,3) = first_rotation_transform*rot_to_start;

  fx += MANYPOINTS_COEFF_INIT_TAN*exp(MANYPOINTS_COEFF_INIT_TAN_EXP*(1.0-opt_params_many_points.tangent_start.dot(newTan)));
  //fx += MANYPOINTS_COEFF_INIT_TAN*pow(1.0-opt_params_many_points.tangent_start.dot(newTan),2);
//  std::cout << "fx tan: " << (MANYPOINTS_COEFF_INIT_TAN*exp(1.0-opt_params_many_points.tangent_start.dot(newTan))) << std::endl;

  //transformFromEulerAngles(transform_to_end, x(ndim-2), x(ndim-1), x(ndim));
  
  //std::cout << "OPT START TAN: \n" << transform_to_end.corner(Eigen::TopLeft,3,1) << std::endl;
  ...
  double curr_z = transform_to_end(2,3);
	...
	for (int i=1; i <= (ndim-3)/2; i++)
	{
      ...
      if (i%opt_params_many_points.num_pts_between == 0)
      {
        ...
        point_error_energy += ((new_end_position - opt_params_many_points.points.at(i/opt_params_many_points.num_pts_between-1))*opt_params_many_points.total_length).squaredNorm();
        //std::cout << "new end: " << new_end_position << std::endl;
        //std::cout << "opt_params_many_points: " << opt_params_many_points.points.at(i/opt_params_many_points.num_pts_between-1) << std::endl;
      }

	}

  //std::cout << "intermediate point error: " << (point_error_energy*opt_params_many_points.thread_intermediate_position_error_penalty) << std::endl;

 // std::cout << "grav energy " << grav_energy*opt_params.gravity_multipler << std::endl;
  fx += grav_energy*opt_params_many_points.gravity_multipler +
          point_error_energy*opt_params_many_points.thread_intermediate_position_error_penalty;

  //std::cout << "OPT END TRANSFORM: \n" << transform_to_end << std::endl;

  //std::cout << "fx before end: " << fx << std::endl;

	Vector3d new_end_position = transform_to_end.corner(Eigen::TopRight,3,1);

	//error in position
	double pos_err = /*opt_params_many_points.total_length**/opt_params_many_points.thread_position_error_penalty*(pow( ((new_end_position - opt_params_many_points.position_end).norm()),2));


  Vector3d endTan = transform_to_end.corner(Eigen::TopLeft,3,1);
  endTan.normalize();
  double end_tan_err = MANYPOINTS_COEFF_END_TAN*exp(MANYPOINTS_COEFF_END_TAN_EXP*(1.0-opt_params_many_points.tangent_end.dot(endTan)));
 // std::cout << "end tan err: " << end_tan_err << std::endl;

  fx += end_tan_err;

//	std::cout << "position error: " << new_end_position << " - " << opt_params_many_points.position_end << " ->" << pos_err << std::endl;
 ...
 // std::cout << "fx AFTER end: " << fx << std::endl;
	//double twist_err = CONSTANT_TWIST_ERROR_PENALTY*(abs(twist_total-opt_params.twist_total));
	//fx += twist_err;
  //std::cout << "fx tot: " << fx << std::endl;
...
Thread_Motion::Thread_Motion(const Vector3d& startPos, const Vector3d& startTan, const Vector3d& endPos, const Vector3d& endTan)
{
  pos_movement = endPos-startPos;
  rotationBetweenTans(startTan, endTan, tan_rotation);
}
...
Thread* Thread_Motion::applyMotion(const Thread* start)
{
  ...
  */
  end->minimize_energy_fixedPieces();
  return end;
}

void Thread_Motion::applyMotion_NoCopy(Thread* thread)
{
  thread->moveEndConstraint(pos_movement, tan_rotation);
