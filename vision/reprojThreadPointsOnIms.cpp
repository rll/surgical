...
#include "../threadenergy/trajectory_reader.h"
...
vector<savedThread>* savedThreads_file;
vector<Thread*>* threads_file;
...
/*  int me, numprocs;
 ...
 sprintf(motions_name, "%s", TRAJ_FILE_PATH);
 sprintf(thread_name, "%s.txt", IMAGE_BASE_NAME);
 sprintf(points_name, "%s.points", IMAGE_BASE_NAME);


  traj_reader = new Trajectory_Reader(motions_name, thread_name, points_name);
  traj_reader->read_motions_from_file();
  traj_reader->read_threads_from_file();

  savedThreads_file = traj_reader->get_all_points();
  threads_file = traj_reader->get_all_threads();

    Scalar* colorsForParams[3];
    colorsForParams[1] = new Scalar(200,0,0);
    colorsForParams[2] = new Scalar(0,200,200);
    colorsForParams[0] = new Scalar(200,0,200);



  for (int i=0; i < threads_file->size(); i++)
  {
    std::cout << i << std::endl;
    threadStereo->myThread.initializeThreadSearch();
    MatrixXd pointsFromVis = savedThreads_file->at(i).points;
    Scalar colorPointsFromVis(200,0,200);
    //threadStereo->addThreadPointsToDebugImages(pointsFromVis, colorPointsFromVis);

    positions[0] = savedThreads_file->at(i).positions[0];
    positions[1] = savedThreads_file->at(i).positions[1];

    tangents[0] = savedThreads_file->at(i).tangents[0];
    tangents[1] = savedThreads_file->at(i).tangents[1];

    MatrixXd pointsFromThreads(17,3);
    length[0] = length[1] = traj_reader->length()/2.0;


    //START DIFF PARAMS
    char energyParamfiles[3][256];
    strcpy(energyParamfiles[1], "/home/pabbeel/rll/code/trunk/surgical/threadenergy/results3/ours_black_124perfect_35perfect/end.params");
    strcpy(energyParamfiles[2], "/home/pabbeel/rll/code/trunk/surgical/threadenergy/results3/ours_nylon_12good4perfect_35good/end.params");
    strcpy(energyParamfiles[0], "/home/pabbeel/rll/code/trunk/surgical/threadenergy/results3/ours_purple_123perfect_45perfect/end.params");

    for (int j=0; j < 3; j++)
    {
      Thread* tominim = new Thread(threads_file->at(i));
      tominim->initEnergyParamsFromFile(energyParamfiles[j]);
      tominim->minimize_energy_fixedPieces();
      MatrixXd pointstominim(17,3);
      tominim->getPoints(pointstominim);
      threadStereo->addThreadPointsToDebugImages(pointstominim, *colorsForParams[j]);
    }
    ...
    //Thread* optimizedThreadFromPoints = new Thread(threads_file->at(i));
    //Thread* optimizedThreadFromPoints = new Thread(curvature, torsion, length, 2, positions, tangents);
    //optimizedThreadFromPoints->optimizeManyPoints_startAndEnd_MyParams(pointsFromVis,2,savedThreads_file->at(i).positions, savedThreads_file->at(i).tangents);
    
   // optimizedThreadFromPoints->getPoints(pointsFromThreads);
    //threads_file->at(i)->getPoints(pointsFromThreads);
    Scalar colorThreadMinimizerOnIms(127,127,0);
    //threadStereo->addThreadPointsToDebugImages(pointsFromThreads, colorThreadMinimizerOnIms);

/*
  std::cout << "points orig     points new      norm\n";
  for (int i=0; i < pointsFromVis.rows(); i++)
  {
    std::cout << (pointsFromVis.block(i,0,1,3)) << "               " << (pointsFromThreads.block(i,0,1,3)) << "      " << (pointsFromVis.block(i,0,1,3) - pointsFromThreads.block(i,0,1,3)).squaredNorm() << std::endl;
  }

 //     std::cout << "points orig \n" << savedIntermediateThreads[currIndIntermediateThread+1] << std::endl;
  //    std::cout << "points new \n" << ptsNewPiece << std::endl;
  double dist = avgDistBetweenPoints(pointsFromVis, pointsFromThreads);
  std::cout << "this dist: " << dist << std::endl;

    
    Vector3d posSimulator[2];
    optimizedThreadFromPoints->getWantedStartPosition(posSimulator[0]);
    optimizedThreadFromPoints->getWantedEndPosition(posSimulator[1]);
    Vector3d tanSimulator[2];
    optimizedThreadFromPoints->getWantedStartTangent(tanSimulator[0]);
    optimizedThreadFromPoints->getWantedEndTangent(tanSimulator[1]);
    Vector3d tanSimulatorActual[3];
    optimizedThreadFromPoints->getActualStartTangent(tanSimulatorActual[0]);
    optimizedThreadFromPoints->getActualEndTangent(tanSimulatorActual[1]);

    positions[0] = savedThreads_file->at(i).positions[0];
    positions[1] = savedThreads_file->at(i).positions[1];


    std::cout << "vision wanted pos: \n" << positions[0] << "\n" << positions[1] << std::endl;
    std::cout << "simulator wanted pos: \n" << posSimulator[0] << "\n" << posSimulator[1] << std::endl;


    std::cout << "vision wanted tan: \n" <<  tangents[0] << "\n" << tangents[1] << std::endl;
    std::cout << "simulator wanted tan: \n" << tanSimulator[0] << "\n" << tanSimulator[1] << std::endl;
    std::cout << "simulator actual tan: \n" << tanSimulatorActual[0] << "\n" << tanSimulatorActual[1] << std::endl;


    Thread* energy_minimized = new Thread(optimizedThreadFromPoints);
    //energy_minimized->minimize_energy_fixedPieces();
    MatrixXd pointsEnergyMinimized(17,3);
    energy_minimized->getPoints(pointsEnergyMinimized);
    Scalar colorThreadMinimized(0,0,255);
   // threadStereo->addThreadPointsToDebugImages(pointsEnergyMinimized, colorThreadMinimized);
*/
