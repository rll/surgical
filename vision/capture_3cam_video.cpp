...
//  c1->init("parameters/");
 ...
 // list<Mat> imsSavedCam3;
  ...
  sprintf(filename, "/media/ssd1/captures/%s1.avi", argv[1]);
  //vid1.open(filename,CV_FOURCC('I', '2', '6', '3'), 25, _frames[0].size(), true);
  vid1.open(filename,CV_FOURCC('I', '4', '2', '0') , 5, _frames[0].size());
  sprintf(filename, "/media/ssd1/captures/%s2.avi", argv[1]);
  //vid2.open(filename,CV_FOURCC('I', '2', '6', '3'), 25, _frames[1].size(), true);
  vid2.open(filename,CV_FOURCC('I', '4', '2', '0') , 5, _frames[1].size());
  sprintf(filename, "/media/ssd1/captures/%s3.avi", argv[1]);
  //vid3.open(filename,CV_FOURCC('I', '2', '6', '3'), 25, _frames[2].size(), true);
  vid3.open(filename,CV_FOURCC('I', '4', '2', '0') , 5, _frames[2].size());
  ...
  for(;;) {
   ...
   //imshow("Example Display3", _frames[2]);
    ...
    if (c == 'c' || continuous == true)
    {
      ...
      imwrite(filename,_frames[0]);
      sprintf(filename, "/media/ssd1/captures/%s2-%d.png", argv[1],f);
      imwrite(filename,_frames[1]);
      sprintf(filename, "/media/ssd1/captures/%s3-%d.png", argv[1],f);
      imwrite(filename,_frames[2]);
      f++;*/
      vid1 << _frames[0];
      vid2 << _frames[1];
      vid3 << _frames[2];
