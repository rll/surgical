...
*/
  ...
  if (argc < 2) {
    cerr << "Usage: example_capture <name>" << endl;
    return 1;
  }
...
//  namedWindow("Example Display3", CV_WINDOW_AUTOSIZE);
  ...
  _cams->updateTimestamps();
  _timestamps = _cams->timestamps();
  ...
  //FILE* timestampFile[NUMCAMS];
  //ofstream byteFilesStreams[NUMCAMS];
  //int byteFilesDesc[NUMCAMS];
  for (int camNum=0; camNum < NUMCAMS; camNum++)
  {
    if (camNum == 0)
...
//    fseek(byteFiles[camNum],0, SEEK_SET);
    //byteFilesStreams[camNum].open(filename, ofstream::binary);
    //fclose (byteFiles[camNum]);
    //byteFilesDesc[camNum] = open(filename, O_WRONLY | O_TRUNC);
    
    //sprintf(filename, "/media/ssd1/captures/%s%d_times.bin", argv[1],(camNum+1));
   // timestampFile[camNum] = fopen(filename, "w");
  }
...
*/
      ...
      for (int camNum=0; camNum < NUMCAMS; camNum++)
      {
        ...
        //if (f % (camNum+1) == 0)
        //byteFilesStreams[camNum].write((char*)_frames[camNum].data, bytesPerIm);
        //byteFilesStreams[camNum].flush();
        //write(byteFilesDesc[camNum], _frames[camNum].data, bytesPerIm);
        //
        //fprintf(timestampFile[camNum], "%f\n", _timestamps[camNum]);
        fwrite(&_timestamps[camNum], sizeof(double), 1, byteFiles[camNum]);


        fflush(byteFiles[camNum]);
      }
      f++;
  ...
  for (int camNum=0; camNum < NUMCAMS; camNum++)
  {
    ...
    {
      sprintf(filename, "/media/ssd1/captures/%s%d-%d.tif", argv[1],camNum+1,f);
      imwrite(filename,*it);
      f++;
    }
  }
...
*/
  ...
  {
		fclose(byteFiles[camNum]);
    //byteFilesStreams[camNum].close();
    //close(byteFilesDesc[camNum]);
	}
