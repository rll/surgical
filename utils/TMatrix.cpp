// #ifdef WIN32 // then no lapack available, have c implementation here
...
void svdcmp(double **a, int m, int n, double *w, double **v, int *failed_p = (int*)NULL)
{
  ...
  if (m < n)
    std::cerr << "SVDCMP: (Probable internal error) "
      "You must augment A with extra zero rows" << std::endl;
  rv1= new double[n];
  for (i=0;i<n;i++) {
    l=i+1;
    rv1[i]=scale*g;
