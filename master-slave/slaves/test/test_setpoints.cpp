...
UDPSocket _buddy_incoming, _buddy_outgoing;
...
while(newRecording){
    ...
    _buddy_incoming = UDPSocket();
    _buddy_outgoing = UDPSocket();
    if (!_buddy_incoming.create()) 
        std::cout << "error creating server socket" << std::endl;
    if (!_buddy_incoming.bind(BUDDY_INCOMING_PORT))
        std::cout << "error binding to port " << BUDDY_INCOMING_PORT << std::endl;
    if (!_buddy_outgoing.create()) 
        std::cout << "error creating server socket" << std::endl;
//    if (!_buddy_outgoing.bind(BUDDY_OUTGOING_PORT))
//        std::cout << "error binding to port " << BUDDY_OUTGOING_PORT << std::endl;
    _buddy_incoming.set_non_blocking(true);
    _buddy_incoming.set_timeout(0);
    _buddy_outgoing.set_non_blocking(true);
    _buddy_outgoing.set_timeout(0);
    _buddy_outgoing.setDestination(BUDDY_IP, BUDDY_INCOMING_PORT); //FIXME 
        
    if (USE_GALIL_PID && NO_VOLTAGES) { 
        printf("\nInitializing PID for Slave 1 (15 seconds)...");
        slaves[0]->_sys->init_pid_control();
        printf("\n\t\t...initialized!\n");
        printf("\nInitializing PID for Slave 2 (15 seconds)...");
        slaves[1]->_sys->init_pid_control();
        printf("\n\t\t...initialized!\n");
    }
    ...
    while(1) {
        if(USE_SLAVE_1)
            //printf("slave 1: %f %f %f %f %f %f %f\n",setpoints[0][0], setpoints[0][1], setpoints[0][2],setpoints[0][3], setpoints[0][4], setpoints[0][5], setpoints[0][6]);
            slaves[0]->move_to(setpoints[0]);
        if(USE_SLAVE_2) 
            //printf("slave 2: %f %f %f %f %f %f %f\n",setpoints[1][0], setpoints[1][1], setpoints[1][2],setpoints[1][3], setpoints[1][4], setpoints[1][5], setpoints[1][6]);
        ...
        if(MASTERS) {
            ...
            while(masters[0]->_accepted && masters[1]->_accepted) {
                if (first_run) {
                    ...
                    masters[0]->create_offsets(
                            ...
                            setpoints[0][5],
                            setpoints[0][6],
                            fabs(slave1_cone_sphere_bound*slave1_radius_height_ratio/(MAX_XY*1.1)));
                    ...
                    masters[1]->create_offsets(
                            ...
                            fabs(slave2_cone_sphere_bound*slave2_radius_height_ratio/(MAX_XY*1.1)));
            ...
            //Messaging::send_message(_outgoing, Messaging::ADJUST_POSE_REQUEST, 1,3,5,7,9,11);
            //if (Messaging::recieve_message(_incoming, Messaging::
...
void print_slave_info(double setpoints[num_slaves][num_dof]){
    ...
    //return; //FIXME
    double kdl_setpoints[num_slaves][num_dof];
    for (int i = 0; i < num_slaves; i++) { 
        for (int k = 0; k < num_dof; k++) {
            kdl_setpoints[i][k] = setpoints[i][k];
        }
    }
    ((Slave_accelera*) slaves[0])->nearest_legal_point(kdl_setpoints[0], kdl_setpoints[0]);
    ((Slave_accelera*) slaves[1])->nearest_legal_point(kdl_setpoints[1], kdl_setpoints[1]);
    double kdl_motors[num_slaves][num_dof];
    ((Slave_accelera*) slaves[0])->point_to_motors(kdl_setpoints[0], kdl_motors[0]);
    ((Slave_accelera*) slaves[1])->point_to_motors(kdl_setpoints[1], kdl_motors[1]);
    ((Slave_accelera*) slaves[0])->motors_to_point(kdl_motors[0], kdl_setpoints[0],false,true);
    ((Slave_accelera*) slaves[1])->motors_to_point(kdl_motors[1], kdl_setpoints[1],false,true);
    for(int j=0;j<num_slaves;j++) {
        if(j==0 && USE_SLAVE_1 || j==1 && USE_SLAVE_2){
            printf("set   %d: ", j+1);
            ...
            for(int i=0;i<num_dof;i++) {
                //printf("[%d] %f, ", i, setpoints[j][i]);
                printf("[%d] %f, ", i, kdl_setpoints[j][i]);
            }
            

            double motors[num_actuators];
            //slaves[j]->point_to_motors(setpoints[j],motors);
            slaves[j]->point_to_motors(kdl_setpoints[j],motors,false,true);
            printf("\nsetm  %d: ",j+1);
