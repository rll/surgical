...
class Accelera_geometry{
    ...
    ~Accelera_geometry();
    //void test_kdl_forward(JntArray joints);
    //void test_kdl(Frame F_dest);
    double* point_to_motors(const double in[num_dof], double out[num_actuators], bool use_elbow_coor=false, bool use_kdl=false);
    double* motors_to_point(const double motor_pos[num_actuators], double out[num_dof], bool use_elbow_coor=false, bool use_kdl=false);
    ...
    int FrameToJnt (const JntArray& q_init, const Frame& p_in, JntArray& q_out, int maxiter, double eps);
    double* convert_point_to_kdl(double in[num_dof], double out[num_dof]);
    double* CheckerboardToRobot(const double in[num_dof], double out[num_dof]);
